...
//HANSを利用して指の座標で物体を操作するために変数teninshiki,teninshiki_camera,teninshiki_camera_yを用意
//HANSを利用して指の座標の変化で物体を操作するために配列hand_x,hand_yを宣言
//変数cameraはTHREE.jsのオブジェクトを代入するため、オブジェクトとして宣言。
let teninshiki = 0;
let teninshiki_camera = 0;
let teninshiki_camera_y = 0;
let teninshiki_camera_z = 0;
let hand_x = [];
let hand_y = [];
let camera = {};
    ...
    hands.onResults(results => {
        if(results.multiHandLandmarks) {
            results.multiHandLandmarks.forEach(marks => {
                ...
                //手を認識し、親指の先端（[0][4]）と人差し指の先端([0][8])のx座標の大小によって、
                //変数teninshikiに渡す値を変える
                ...
                //forEach実行ごとの人差し指の先端([0][8])のx座標を配列hand_xに入れていく
                hand_x.push(results.multiHandLandmarks[0][8].x);
                //今の人差し指の先端([0][8])のx座標を、２回前の繰り返し時点のx座標と比較して、
                //大きければteninshiki_cameraに負の値を、小さければ正の値を入れる。
                ...
                //forEach実行ごとの人差し指の先端([0][8])のy座標を配列hand_yに入れていく
                hand_y.push(results.multiHandLandmarks[0][8].y);
                //今の人差し指の先端([0][8])のy座標を、２回前の繰り返し時点のy座標と比較して、
                //大きければteninshiki_camera_yに正の値を、小さければ負の値を入れる。
      ...
      //THREE.jsのカメラオブジェクトを初期位置に戻すための処理
...
function moji_clone(moji){
    if(moji == 'A' ||moji == 'a'){return letter_A.clone();}
    if(moji == 'B' ||moji == 'b'){return letter_B.clone();}
    if(moji == 'C' ||moji == 'c'){return letter_C.clone();}
    if(moji == 'D' ||moji == 'd'){return letter_D.clone();}
    if(moji == 'E' ||moji == 'e'){return letter_E.clone();}
    if(moji == 'F' ||moji == 'f'){return letter_F.clone();}
    if(moji == 'G' ||moji == 'g'){return letter_G.clone();}
    if(moji == 'H' ||moji == 'h'){return letter_H.clone();}
    if(moji == 'I' ||moji == 'i'){return letter_I.clone();}
    if(moji == 'J' ||moji == 'j'){return letter_J.clone();}
    if(moji == 'K' ||moji == 'k'){return letter_K.clone();}
    if(moji == 'L' ||moji == 'l'){return letter_L.clone();}
    if(moji == 'M' ||moji == 'm'){return letter_M.clone();}
    if(moji == 'N' ||moji == 'n'){return letter_N.clone();}
    if(moji == 'O' ||moji == 'o'){return letter_O.clone();}
    if(moji == 'P' ||moji == 'p'){return letter_P.clone();}
    if(moji == 'Q' ||moji == 'q'){return letter_Q.clone();}
    if(moji == 'R' ||moji == 'r'){return letter_R.clone();}
    if(moji == 'S' ||moji == 's'){return letter_S.clone();}
    if(moji == 'T' ||moji == 't'){return letter_T.clone();}
    if(moji == 'U' ||moji == 'u'){return letter_U.clone();}
    if(moji == 'V' ||moji == 'v'){return letter_V.clone();}
    if(moji == 'W' ||moji == 'w'){return letter_W.clone();}
    if(moji == 'X' ||moji == 'x'){return letter_X.clone();}
    if(moji == 'Y' ||moji == 'y'){return letter_Y.clone();}
    if(moji == 'Z' ||moji == 'g'){return letter_Z.clone();}
}


renda_suru_moji = new THREE.Group();
...
send_char.addEventListener('click', function(){
    //受け取った文字を１文字づつ配列に格納
    get_shita_moji = input_char.value.split('');

    if(get_shita_moji.length%2 == 0){
        for(let i = 0;i < get_shita_moji.length; i++){
            three_letter = moji_clone(get_shita_moji[i]);
            three_letter.position.set(-2.5-(get_shita_moji.length/2-(1+i))*5+i,0,0);
            console.log(-2.5-(get_shita_moji.length/2-(1+i))*5+1);
            renda_suru_moji.add(three_letter);
        }
    }else{
        for(let i = 0;i < get_shita_moji.length; i++){
            three_letter = moji_clone(get_shita_moji[i]);
            three_letter.position.set(-(Math.floor(get_shita_moji.length/2)-i)*5+i,0,0);
            console.log(-(Math.floor(get_shita_moji.length/2)-i)*5);
            renda_suru_moji.add(three_letter);
        }
    }
    scene.add(renda_suru_moji);
});
    ...
    function tick() {
        // レンダリング
        renda_suru_moji.rotation.y += Math.PI/180*teninshiki;
